#include "OmtfTreeAnalysis_gen.h"
...
OmtfTreeAnalysis_gen::OmtfTreeAnalysis_gen(const edm::ParameterSet & cfg)
  : theConfig(cfg),
    theAnaEvent(0), 
...
void OmtfTreeAnalysis_gen::beginJob()
{
  theHistos.SetOwner();
...
void OmtfTreeAnalysis_gen::beginRun(const edm::Run& ru, const edm::EventSetup& es)
{
  std::cout <<" OmtfTreeAnalysis_gen::beginRun called" << std::endl;
  if (theAnaSynch) theAnaSynch->beginRun(ru,es);

}

void OmtfTreeAnalysis_gen::analyze(const edm::Event&, const edm::EventSetup& es)
{
  //
  ...
  if (the_EMERGENCY_STOP) {
    std::cout <<"**  OmtfTreeAnalysis_gen::analyze EMERGENCY_STOP fired, skip" << std::endl;
    return;
  }
...
//    std::cout <<"---------------------------------------#"<<ev<<", event: "<< *event << std::endl;
    ...
    GenObj muon;
    if (theAnaSecMu) {
       muon = theAnaSecMu->run(event, *genColl, l1ObjColl);
    } else { 
      std::vector<GenObj> muons = *genColl; 
...
*/
    ...
    if (theAnaTime)     theAnaTime->run( event, genColl, closestTrack, l1ObjColl);
    if (theAnaSynch)    theAnaSynch->run( event, ConverterRPCRawSynchroSynchroCountsObj::toRawSynchro( synchroCounts->data));
...
void OmtfTreeAnalysis_gen::endJob()
{
