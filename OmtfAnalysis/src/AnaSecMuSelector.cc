...
GenObj AnaSecMuSelector::run( 
    const EventObj* ev, const std::vector<GenObj> & muons, const L1ObjColl * l1Objs) 
{
  // New version running with gen level objets as the muon collection.
  // Retrieves a probe object for a tag & probe -like analysis using a L1T-matched muon as the tag
  // First check if there is a gen muon deltaR matched to a L1 object
  //
  GenObj trg;
  unsigned int imuon =0;
  for (const auto & muon : muons) {
    ...
    if (   theCfgTrg.exists("requireOutsideOMTF")
        && theCfgTrg.getParameter<bool>("requireOutsideOMTF") && !(fabs(muon.eta()) < 0.6 || fabs(muon.eta())> 1.5) ) continue;
    //ID + HLT requirements are currently commented while working on what is feasible to do to get something that compiles. TODO:: Final update!
    //if (theCfgTrg.getParameter<bool>("requireTight") && !muon.isTight) continue;
    //#if (theCfgTrg.getParameter<bool>("requireUnique") && !muon.isUnique) continue;
    //if (    theCfgTrg.getParameter<bool>("requireHLT")  
    //     && !muon.isMatchedHlt && !muon.isMatchedIsoHlt ) continue;
  ...
  if (!trg.isValid()) return GenObj();

  //
  // Now look for a probe gen muon that passes minimum quality criteria and is separated to the triggering one
  //
  GenObj prb;
  for (const auto & probe : muons) {
    if (   theCfgProbe.exists("requireCharge")
  ...
  if (prb.isValid()) {
    //Angular differences between the triggering tag muon and the probe one
    hSecMuPtDis->Fill(trg.pt(), prb.pt());
    hSecMuEtaDis->Fill(trg.eta(), prb.eta());
    hSecMuDeltaR->Fill(reco::deltaR( trg.eta(), trg.phi(), prb.eta(), prb.phi()));
    hSecMuDeltaEta->Fill(trg.eta()-prb.eta());
